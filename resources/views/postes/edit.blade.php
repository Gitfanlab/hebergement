<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight flex justify-between">
            {{ __('Modifier un poste') }} <a href="{{ route('postes.index') }}"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('postes.update', $poste) }}"> @csrf
                        @method('PUT') <div>
                            <x-input-label for="code" :value="__('Code')" />
                            <x-text-input id="code" class="block mt-1 w-full mb-4" type="text" name="code"
                                :value="$poste->code" required />
                            @error('code')
                        ...
                        <div>
                            ...
                            <select id="type_acces" name="type_acces"
                                ...
                                <option value="BADGE"
                                    {{ old('type_acces', $poste->type_acces) == 'BADGE' ? 'selected' : '' }}>BADGE
                                </option>
                                <option value="CODE"
                                    {{ old('type_acces', $poste->type_acces) == 'CODE' ? 'selected' : '' }}>CODE
                                </option>
                            </select>
                            @error('type_acces')
                                <p class="text-red-500">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <x-text-input id="code_acces" class="block mt-1 w-full mb-4" type="text"
                                name="code_acces" :value="$poste->code_acces" />
                            @error('code_acces')
                                <p class="text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="sexe" name="sexe"
                                class="block mt-1 w-full mb-4 rounded border-gray-300">
                                <option value="" {{ empty($poste->sexe) ? 'selected' : '' }}>Sélectionnez un sexe
                                </option>
                                <option value="Masculin" {{ $poste->sexe == 'Masculin' ? 'selected' : '' }}>Masculin
                                </option>
                                <option value="Féminin" {{ $poste->sexe == 'Féminin' ? 'selected' : '' }}>Féminin
                                </option>
                            </select>
                            @error('sexe')
                                <p class="text-red-500">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <x-text-input id="groupement" class="block mt-1 w-full mb-4" type="text"
                                name="groupement" :value="$poste->groupement" required />
                            @error('groupement')
                                <p class="text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="type_renfort" class="block mt-1 w-full mb-4" type="text"
                                name="type_renfort" :value="$poste->type_renfort" />
                            @error('type_renfort')
                                <p class="text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="observation" name="observation"
                                class="block mt-1 w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">{{ $poste->observation }}</textarea>
                            @error('observation')
                                <p class="text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="modif_annee_pro" class="block mt-1 w-full mb-4" type="text"
                                name="modif_annee_pro" :value="$poste->modif_annee_pro" />
                            ...
                            <select id="grade_id" class="block mt-1 w-full mb-4 rounded border-gray-300"
                                ...
                                @foreach ($grades as $grade)
                                    <option value="{{ $grade->id }}"
                                        {{ $poste->grade_id == $grade->id ? 'selected' : '' }}>{{ $grade->nom }}
                                    </option>
                                @endforeach
                            </select>
                            @error('grade_id')
