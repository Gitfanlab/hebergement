        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('personnels.update', $personnel) }}">
                        @method('PUT')
                        @csrf
                        ...
                        <div>
                            ...
                            <x-text-input id="nom" class="block mt-1 w-full mb-4" type="text" name="nom"
                                :value="$personnel->nom" required autofocus />
                            @error('nom')
                        ...
                        <div>
                            ...
                            <x-text-input id="prenom" class="block mt-1 w-full mb-4" type="text" name="prenom"
                                :value="$personnel->prenom" required />
                            @error('prenom')
                                <p class="text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="sexe" name="sexe"
                                ...
                                <option value="" {{ empty($personnel->sexe) ? 'selected' : '' }} class="hidden">
                                    Sélectionnez un sexe</option>
                                <option value="Masculin" {{ $personnel->sexe == 'Masculin' ? 'selected' : '' }}>Masculin
                                </option>
                                <option value="Féminin" {{ $personnel->sexe == 'Féminin' ? 'selected' : '' }}>Féminin
                                </option>
                            </select>
                            @error('sexe')
                                <p class="text-red-500">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <x-text-input id="groupement" class="block mt-1 w-full mb-4" type="text"
                                name="groupement" :value="$personnel->groupement" />
                            @error('groupement')
                                <p class="text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <select id="grade_id" class="block mt-1 w-full mb-4 rounded border-gray-300"
                                ...
                                @foreach ($grades as $grade)
                                    <option value="{{ $grade->id }}"
                                        {{ $personnel->grade_id == $grade->id ? 'selected' : '' }}>{{ $grade->nom }}
                                    </option>
                                @endforeach
                            </select>
                            @error('grade_id')
